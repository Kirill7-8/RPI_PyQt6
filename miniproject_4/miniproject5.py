...
class proj4(QMainWindow):
    def __init__(self):
        ...
        self.data = json.load(self.data.value(login, "[]"))
...
#data = [{"theme" : "theme 1","za": ["aaaaaa", "bbbbbb"],"no": ["ccccc", "ddddd"]}, {"theme": "theme 2","za": ["12345", "12"],"no": ["no", "yeeeeeeee"]}]
    ...
    def cur_tabs(self):   
        if self.data:
            for theme in self.data:
                new_tab = proj4(theme["za"], theme["no"], theme["theme"])  
